...
function TodoList() {
  ...
  return (
    <div>
      ...
      {open && <Popup state={[open, setOpen]}/>}
      {todos.length === 0 ? <p> Add your todos! </p> :
        <ul className="todo-container">
          {todos.map(({ id, todo }) => {
            return <li> {todo} <button className="add" onClick={e => db.collection('todos').doc(id).delete()}> Delete </button> <button onClick={() => setOpen(!open)}> Edit </button></li>
          })}
        </ul>}
    </div>
  )
        ...
        <div className="modal-frame">
          ...
          <div className="modal-info">
            <div className="modal-text">
              ...
              <p> STUDYBOT is the ultimate application if you want to achieve success in school or work that contains multiple features that helps you maximize your productivity. This includes a Pomodoro timer to track your studying time, a bad posture detector that detects bad posture during studying, and finally a virtual bot that can play music, inspire you and provide you with resources. Lastly, all of your performances will be tracked and sent to your email so you can progressively enhance your studying habits and achieve things you wish. </p>
              <h5 style={{ fontWeight: 800 }}> Happy Studying!</h5>
            </div>
