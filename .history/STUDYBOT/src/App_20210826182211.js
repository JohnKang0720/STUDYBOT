      ...
      {todos.length === 0 ? <p> Add your todos! </p> :
        <ul className="todo-container">
          {todos.map(({ id, todo, priority }) => {
            return <li> {todo}  <button className="add" onClick={e => db.collection('todos').doc(id).delete()}> Delete </button> <button className="add" onClick={() => setOpen(!open)}> Edit </button> {open && <Popup state={[open, setOpen, id]} />}</li>
          })}
        </ul>}
    </div>
  )
