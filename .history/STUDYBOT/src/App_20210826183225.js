      ...
      {todos.length === 0 ? <p> Add your todos! </p> :
        <ul className="todo-container">
          {todos.map(({ id, todo, priority }) => {
            return <li> <Todo todolist={todo} state={[open, setOpen]}/>  </li>
          })}
        </ul>}
    </div>
  )
...
function Todo(props) {
  return (
    <>
      <p> {props.todolist} </p>
      <button className="add" onClick={e => db.collection('todos').doc(props.todolist.id).delete()}> Delete </button> <button className="add" onClick={() => props.state.setOpen(!props.state.open)}> Edit </button>
