...
function Todo(props) {
  ...
  return (
    <>
      <p> {todo} </p>
      <button className="add" onClick={e => db.collection('todos').doc(id).delete()}> Delete </button> <button className="add" onClick={() => props.state.setOpen(!props.state.open)}> Edit </button>
      {props.state.open && <Popup/>}
    </>
  )
}
