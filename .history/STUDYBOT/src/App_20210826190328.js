      ...
      {todos.length === 0 ? <p> Add your todos! </p> :
        <ul className="todo-container">
          {todos.map(({ id, todo, priority }) => {
            return <li> <Todo todolist={[todo, id]} state={[open, setOpen]} />  </li>
          })}
        </ul>}
    </div>
  )
...
function Todo(props) {
  ...
  return (
    <>
      <p> {todo} </p>
      <button className="add" onClick={e => db.collection('todos').doc(id).delete()}> Delete </button> <button className="add" onClick={() => setOpens(!opens)}> Edit </button>
...
//   const [open, setOpen, id] = props.states
//   const [edit, setEdit] = useState('')
//   const [prio, setPrio] = useState('')
