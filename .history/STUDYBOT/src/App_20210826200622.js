...
function App() {
  ...
  function read(m) {
    ...
    if (m.includes('quotes')) {
      ...
      //reads out # of todos and todos
    } else if (m.includes("how am i doing")) {
...
function TodoList() {
  ...
  useEffect(() => {
    db.collection('todos').orderBy('timestamp', 'desc').onSnapshot(snapshot => {
      ...
      setTodos(snapshot.docs.map(doc => ({ id: doc.id, todo: doc.data().todo, priority: doc.data().priority })))
    })
  }, [input])
  ...
  return (
    <div className="list-container">
      ...
      <Button className="add" variant="contained" color="primary" sonClick={handleAdd}> Add Todos </Button>
      {todos.length === 0 ? <p> Add your todos! </p> :
        <ul className="todo-container">
          {todos.map(({ id, todo, priority }) => {
            return <li> <Todo todolist={[todo, id, priority]} state={[open, setOpen]} />  </li>
          })}
        </ul>}
    </div>
  )
...
function Todo(props) {
  const [todo, id, priority] = props.todolist
  ...
  return (
    <>
      <p> {todo} -- {priority}</p>
      <button className="add" onClick={e => db.collection('todos').doc(id).delete()}> Delete </button>
      <button type="button" onClick={handleOpen} className="add">
        Edit
      </button>
      <Modal
        open={open}
      ...
      >
        <div style={modalStyle} className={classes.paper}>
          ...
          <input placeholder="Edit your todo..." onChange={e => setEdit(e.target.value)} />
          <input placeholder="Edit your priority..." onChange={e => setPrio(e.target.value)} />
