...
function Todo(props) {
  ...
  return (
    <>
      <p> {todo} -- {priority}</p>
      <IconButton variant="contained" color="secondary" aria-label="delete" onClick={e => db.collection('todos').doc(id).delete()}> Delete </IconButton>
      <CreateIcon type="button"  onClick={handleOpen} className="add">
        Edit
      </CreateIcon>
      <Modal
        open={open}
