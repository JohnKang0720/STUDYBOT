...
localStorage.setItem('final2', JSON.stringify([]))
localStorage.setItem('final3', JSON.stringify([]))
const x = JSON.parse(localStorage.getItem('final'))
const y = JSON.parse(localStorage.getItem('final2'))
const z = JSON.parse(localStorage.getItem('final3'))
...
class Timer extends React.Component {
    ...
    getHighScore(minutes, seconds, rounds) {
        ...
        y.push(rounds)
        y.sort()
        const score = y[y.length - 1]

        z.push(minutes)
        z.sort()
        const hightime = z[z.length - 1]

        localStorage.setItem('highscore', score)
        localStorage.setItem('hightime', hightime)
        localStorage.setItem('hightime2', JSON.stringify(hightime2))

    }
    render() {
        return (
            <div className="timer-inner">
                <p style={{ fontSize: '1.5rem', background: 'orange', margin: '5px', borderRadius: '10px' }}> Todays' high Score: {localStorage.getItem('highscore')}, {localStorage.getItem('hightime')}m : {localStorage.getItem('hightime2')}s </p>
                <p style={{ fontSize: '1.5rem', background: 'orange', margin: '5px', borderRadius: '10px' }}> Rounds Completed: <strong> {this.state.rounds} </strong> </p>
